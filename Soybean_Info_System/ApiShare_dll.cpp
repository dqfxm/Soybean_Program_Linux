...
Share_class::~Share_class()
{
        //cout<<"为保证c++关闭python程序,析构函数不执行内存销毁,**需要由主程序执行**"<<endl;
        //cout<<"主程序执行析构函数销毁共享内存"<<endl;
        cout<<"执行析构函数销毁共享内存"<<endl;
        DestroyShare();
}  //结束销毁函数

//2创建共享内存初始化
int Share_class::InitShare(){

    int shmid = shmget((key_t)Shm_addrees, sizeof(ShareData), 0666|IPC_CREAT);
    //2映射共享内存地址  shm指针记录了起始地址
    shm = shmat(shmid, (void*)0, 0);//如果创建一个函数每次调用都执行，需要执行完释放一下shmdt
    //printf("共享内存地址 ： %p\n", (int *)(shm));
    //2-1以ShareData结构体类型-访问共享内存
    pShareData= (ShareData*)shm;
    //用来保存转化后的共享内存的图像结果
    cvoutImg = cv::Mat(IMAGE_H,IMAGE_W,CV_8UC3,cv::Scalar(255, 255, 255));//bufHight,bufWidth
    printf("共享内存地址 ： %p\n", (int *)(shm));
    return 1;
}

//3销毁共享内存
int  Share_class::DestroyShare(){
    //4断开映射 ，保证下次访问不被占用
    shmdt(shm);
    //5释放共享内存地址
...
int Share_class::Send_pic2_share_once(cv::Mat Img)
{
    int i = 0;
...
int Share_class::pySend_pic2_share_once(uchar *frame_data, int height, int width)
{
  if(pShareData->flag == 0)
    {
...
int Share_class::Rec_pic2_data_once()
{  //cv::Mat &cvoutImg_in
    //3-1共享内存读取标志位
...
*/
uchar* Share_class::Img_Cgg2py()
{  //uchar* frame_data, int rows, int cols, int channels
...
int Share_class::Set_ImgFalg(int value)
{
    pShareData->flag =value;
}


//传输obtctrl[7]数组 接受py数组并修改，返回总结果
//20220324
//fxm
float Share_class::pyarr_set_cgg_obtctrl_share(float *data,int len)
{
    float result=0;
    if (pShareData->obtctrl_flag == 0){
        result=1;
        for (int i = 0; i < len; i++) {
            pShareData->obtctrl[i]=data[i];
        }
        pShareData->obtctrl_flag = 1;  //obtctrl_flag = 1允许读取obtctrl数据，0允许存数据
    }
    return result;
}
...
float Share_class::pyarr_set_cgg_gps_share(float *data, int len)
{
    float result=1;
    for (int i = 0; i < len; i++) {
...
uchar* Share_class::py_get_cgg_gps_share()
{
...
StructGpsData Share_class::py_get_cgg_gps_Struct( StructGpsData gps)
{
    // c++发送端调用其他函数更新GPS数据,保存在共内存（简单举例）
    //pShareData->Gps[0]=1.56;
    //pShareData->Gps[1]=2.34;
    ...
    //共享内存数据更新gps数据
    char message[]="new share data from c++  share";
    gps.flag=1;
    gps.msg = message;
...
*/
StructGpsData Share_class::py_Set_cgg_gps_Struct( StructGpsData gps)
{
// c++发送端调用其他函数更新GPS数据,保存在共内存（简单举例）
    char message[]= "new share have set share c++";
    gps.flag=1;
    gps.msg=message;
...
//设置和获取停止python主循环控制
int Share_class::Set_loop_flag(int value)
{
    pShareData->stopflag = value;
}

int Share_class::Get_loop_flag()
{
    return pShareData->stopflag;
}
...
extern "C" {
    ...
    float pyarr_set_cgg_obtctrl_share_(float *data,int len){
        useShare.pyarr_set_cgg_obtctrl_share( data, len);
    }
    ...
    int Set_loop_flag_(int value){
        useShare.Set_loop_flag(value);
    }

    int Get_loop_flag_(){
        useShare.Get_loop_flag();
    }
}
